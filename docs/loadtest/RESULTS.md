...
- **peak Hikari pending** — despite the column name, translatr's pool is **Agroal**: read
  `agroal_awaiting_count` (threads blocked waiting for a connection). Requires
  `quarkus.datasource.jdbc.enable-metrics=true`; leave blank if not enabled for the run.
