...
test.describe('Project Keys Delete Key Visibility', () => {
  ...
  test('should show key delete button for member role Developer', async ({ page }) => {
    ...
    // then
    await expect(keys.getKeyList().locator('confirm-button.delete')).toHaveCount(2);
  });
  ...
  test('should not show key delete button for member role Translator', async ({ page }) => {
    ...
    // then
    await expect(keys.getKeyList().locator('confirm-button.delete')).toHaveCount(0);
  });
