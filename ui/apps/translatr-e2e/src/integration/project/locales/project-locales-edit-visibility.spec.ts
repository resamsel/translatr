...
test.describe('Project Locales Edit Locale Visibility', () => {
  ...
  test('should not show locale delete button for member role Developer', async ({ page }) => {
    ...
    // then
    await expect(locales.getLocaleList().locator('button.edit')).toHaveCount(0);
  });
  ...
  test('should show locale delete button for member role Translator', async ({ page }) => {
    ...
    // then
    await expect(locales.getLocaleList().locator('button.edit')).toHaveCount(2);
  });
