  ...
  return getRandomUserAccessToken(injector, {limit: '10'}, (user: User) => user.role === UserRole.User)
    .pipe(
      ...
      concatMap((payload: { user: User, project: Project, accessToken: AccessToken }) => {
        return combineLatest(_.sample(localeNames, Math.ceil(Math.random() * 12))
          .map((localeName: string) =>
            createLocale(injector,
              {
                name: localeName,
      ...
      concatMap((payload: { user: User, project: Project, locales: Locale[], accessToken: AccessToken }) => {
        return combineLatest(_.sample(keyNames, Math.ceil(Math.random() * 50))
          .map((keyName: string) =>
            createKey(injector,
              {
                name: keyName,
      ...
      concatMap((payload: { user: User; project: Project; locales: Locale[]; keys: Key[]; accessToken: AccessToken }) => {
        const locale = pickRandomly(payload.locales);
        return combineLatest(payload.keys.map((key: Key) =>
          createMessage(injector,
            {
              localeId: locale.id,
              keyId: key.id,
...
export const getRandomProject = (injector: Injector, user: User, accessToken: AccessToken): Observable<Project> => {
  const projectService = injector.get(ProjectService);
    ...
    .pipe(
      ...
      concatMap((payload: { user: User, accessToken: AccessToken }) =>
        getRandomProject(injector, payload.user, payload.accessToken)
          .pipe(map((project: Project) => ({...payload, project})))),
      filter((payload: { user: User; accessToken: AccessToken; project: Project }) => payload.project !== undefined),
      concatMap((payload: { user: User; accessToken: AccessToken; project: Project }) =>
        injector.get(ProjectService)
          .delete(payload.project.id, {params: {access_token: payload.accessToken.key}})
          .pipe(mapTo(payload)))
    )
    .pipe(map((payload: { user: User; accessToken: AccessToken; project: Project }) =>
