...
export const createKey = (
  ...
  return injector.get(KeyService).withAuth(accessToken.key).create(key);
};
...
export const createRandomKeyForProject = (
  ...
  keyService
    .withAuth(chooseAccessToken(accessToken, defaultAccessToken, Scope.ProjectRead, Scope.KeyRead))
    .find({
    ...
    .pipe(
      ...
      concatMap(({ keys: _keys, keyName }) =>
        keyService
          .withAuth(
            chooseAccessToken(accessToken, defaultAccessToken, Scope.ProjectRead, Scope.KeyWrite),
          )
          .create({
            name: keyName,
            projectId: project.id,
...
export const deleteRandomKey = (
  ...
  return selectRandomProjectAccessToken(
    ...
    concatMap(({ accessToken, project }) =>
      keyService
        .withAuth(
          chooseAccessToken(accessToken, defaultAccessToken, Scope.ProjectRead, Scope.KeyRead),
        )
        .find({
          projectId: project.id,
        })
    ...
    concatMap(({ accessToken, project: _project, keys }) =>
      keyService
        ...
        .delete(pickRandomly(keys.map((key: Key) => key.id))),
    ),
  );
};
