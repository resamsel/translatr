...
export const localeNames = ['en', 'de', 'it', 'fr', 'hu', 'sl', 'cs', 'es', 'pl', 'gr', 'en-US', 'en-UK', 'de-AT', 'de-DE'];

export const createLocale = (injector: Injector, locale: Locale, accessToken: AccessToken): Observable<Locale> => {
  return injector.get(LocaleService).create(locale, {params: {access_token: accessToken.key}});
};

export const deleteLocale = (injector: Injector, localeId: string, accessToken: AccessToken): Observable<Locale> => {
  return injector.get(LocaleService).delete(localeId, {params: {access_token: accessToken.key}});
};

export const createRandomLocale = (injector: Injector): Observable<Partial<State>> => {
  return getRandomUserAccessToken(injector, {}, (user: User) => user.role !== UserRole.Admin)
    .pipe(
      concatMap((payload: { user: User; accessToken: AccessToken }) =>
        getRandomProject(injector, payload.user, payload.accessToken)
          .pipe(map((project: Project) => ({...payload, project})))),
      filter((payload: { user: User; accessToken: AccessToken; project: Project }) =>
        payload.project !== undefined),
      concatMap((payload: { user: User; accessToken: AccessToken; project: Project }) =>
        injector.get(LocaleService)
          .find({projectId: payload.project.id, access_token: payload.accessToken.key})
          .pipe(map((pagedList: PagedList<Locale>) => ({...payload, locales: pagedList.list})))
      ),
      concatMap((payload: { user: User; accessToken: AccessToken; project: Project; locales: Locale[] }) =>
        createLocale(
          injector,
          {
            name: pickRandomly(_.difference(
              localeNames, payload.locales.map((locale: Locale) => locale.name))),
            projectId: payload.project.id
          },
          payload.accessToken
        )
          .pipe(map((locale: Locale) => ({...payload, locale})))),
      map((payload: { user: User; project: Project; locale: Locale }) =>
