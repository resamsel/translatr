...
public class RevertDeleteProjectCommand implements Command<models.Project>
{
	 ...
	 */
	@Inject
	public RevertDeleteProjectCommand(ProjectService projectService, LocaleService localeService, KeyService keyService,
	...
	{
		LOGGER.info(
			"DTO Project #locales: {}, #keys: {}, #messages: {}",
		...
		Map<String, Locale> locales = localeService
			...
			.stream()
			.collect(groupingBy(l -> l.name, reducing(null, a -> a, (a, b) -> b)));
		messageService.save(
			project.messages
				.stream()
