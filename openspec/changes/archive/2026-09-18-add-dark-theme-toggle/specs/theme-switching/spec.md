...
The theme-switchering UI SHALL only be shown to a user when the `ThemeSwitcher` feature flag is enabled for that user. When the flag is disabled, the app SHALL behave exactly as before (fixed light theme, no toggle visible).
